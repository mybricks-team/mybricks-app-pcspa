...
export async function generateCode(toJSON: ToJSON) {
  ...
  const fileName =  `toCode-${Math.random()}`;
  const tempProjectPath = path.join(os.tmpdir(), fileName)
  ...
  const indexEjsCode = await prettier.format(fse.readFileSync(indexEjsCodePath, "utf-8")
    ...
    /** TODO: 临时 */
    // .replace("<!-- replace-dependencies -->", '<script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>'), 
    ,{ parser: "html" }
    )
...
function generateUiComponentCode(component: ComponentNode, { filePath: parentFilePath, scene, frame }:  { filePath: string, scene: ToBaseJSON, frame: Frame}) {
  ...
  const Runtime = globalContext.getComponentDefinition("${namespace}").runtime;
