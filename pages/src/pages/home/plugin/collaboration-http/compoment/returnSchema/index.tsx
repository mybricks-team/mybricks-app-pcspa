...
export default function ReturnSchema({
  ...
  const collaborationHttpContext = useContext(CollaborationHttpContext);
  ...
  const markAsReturn = useCallback(() => {
    const outputKeys = excludeKeysRef.current.some((key) => key === curKeyRef.current) ? keys : [
      ...keys.filter((key: string) => !(key.includes(curKeyRef.current) || curKeyRef.current.includes(key))),
      curKeyRef.current,
    ].filter((key) => key !== '');
    const newExcludeKeys = excludeKeys.filter((key) => key !== curKeyRef.current);

    onKeysChange([...outputKeys], newExcludeKeys);

    setOutputKeys(outputKeys);
    setExcludeKeys(newExcludeKeys);
  }, [keys, excludeKeys, onKeysChange]);
  ...
  const popMark = useCallback((e, xpath) => {
    ...
    collaborationHttpContext.addBlurAry('return-schema', () => setStyle(void 0));
  }, []);

  const cancelMark = useCallback((e, xpath) => {
    const outputKeys = keys.filter((key: string) => key !== xpath && key !== '');
    let newExcludeKeys = [...excludeKeys];

    if (!keys.some((key) => key === xpath)) {
      newExcludeKeys = [
        ...excludeKeys.filter((key) => !(key.includes(xpath) || xpath.includes(key))),
        xpath,
      ]
    }
  ...
  const resetPopMenuStyle = useCallback((event) => {
    setStyle(void 0);
    collaborationHttpContext.addBlurAry('return-schema', () => {});
