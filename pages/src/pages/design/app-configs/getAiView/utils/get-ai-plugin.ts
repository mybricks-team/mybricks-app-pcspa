import AIPlugin, { fileFormat } from '@mybricks/plugin-ai'


export default ({ requestAsStream, user, key, guidePrompt, enableDefaultEventFlow }: any) => AIPlugin({
  requestAsStream,
  user,
  isMutiCanvas: false,
  deviceType: 'desktop',
  prompts: {
    guidePrompt,
    enableDefaultEventFlow,
    canvasWidth: '1024',
    systemAppendPrompts: systemAppendPrompts(),
    prdExamplesPrompts: prdExamplesPrompts(),
    generatePageActionExamplesPrompts: generatePageActionExamplesPrompts(),
  },
  key,
  createTemplates: {
    page: ({ title }) => {
      return {
        type: "normal",
        title: "页面",
        inputs: [
          {
            id: "open",
            title: "打开",
            schema: {
              type: "any",
            },
          },
        ],
      }
    }
  },
//   mock: [
//     `M:\`\`\`bash
// node generate-prd-and-require-component -mode generate && node clear-and-generate-page
// \`\`\`
// 完整性检查：根据用户想要“京东`,
//     `接口返回结果 我来为你生成一个《星露谷物语》Wiki百科页面的需求文档和组件选型。

// \`\`\`md title="星露谷物语Wiki百科需求文档.md"
// **概述**
// 设计一个功能完整的《星露谷物语》游戏Wiki百科页面，为玩家提供全面的游戏攻略、物品图鉴、角色信息和社区交流功能。整体采用田园风格设计，营造温馨自然的游戏氛围。


//   - 左侧筛选面板：包含"全部"、"农作物"、"矿物"、"鱼类"、"料理"、"工具"等分类标签
//   - 右侧物品网格：每行4个物品卡片，每个卡片包含：
//     - 物品图标（64x64px）
//     - 物品名称
//     - 稀有度标签（普通/银星/金星）
//     - 售价信息
//     - 获取方式简述
//   - 底部"查看更多"按钮

// **角色关系图**
// - **功能**：展示游戏中NPC角色信息和好感度系统
// - **视觉**：卡片式布局，每行3个角色卡片
// - **内容**：
//   - 区块标题："角色关系" + 心形装饰图标
//   - 角色卡片列表，每个卡片包含：
//     - 角色头像（圆形，80x80px）
//     - 角色姓名和职业
//     - 好感度等级显示（心形图标）
//     - 喜好物品标签
//     - "查看详情"文本链接

// **农场规划区**
// - **功能**：提供农场布局设计和规划工具
// - **视觉**：左右布局，左侧工具面板，右侧展示区域
// - **内容**：
//   - 区块标题："农场规划" + 房屋装饰图标
//   - 左侧工具栏：包含"布局模板"、"建筑工具"、"装饰物品"等分类按钮
//   - 右侧展示区：
//     - 推荐布局缩略图（3x2网格）
//     - 每个布局包含预览图、布局名称、适用阶段标签
//     - "使用此布局"按钮

// **攻略指南区**
// - **功能**：提供游戏攻略和技巧分享
// - **视觉**：文章列表形式，左侧缩略图，右侧文字信息
// - **内容**：
//   - 区块标题："攻略指南" + 书本装饰图标
//   - 攻略文章列表，每篇文章包含：
//     - 文章缩略图（120x80px）
//     - 文章标题
//     - 文章摘要（2行截断）
//     - 发布时间和阅读量
//     - 难度标签（新手/进阶/高级）

// **社区讨论区**
// - **功能**：玩家交流和讨论平台
// - **视觉**：论坛式布局，话题列表展示
// - **内容**：
//   - 区块标题："社区讨论" + 对话框装饰图标
//   - 热门话题列表，每个话题包含：
//     - 话题标题
//     - 发起人头像和昵称
//     - 回复数量和最后回复时间
//     - 话题分类标签
//   - 右侧边栏：
//     - "发起新话题"按钮
//     - 在线用户数量显示
//     - 今
// **风险提示**
// - **响应式布局**：确保在1024px画布宽度下，所有内容模块能够合理排布，避免内容溢出
// - **图片资源**：物品图标、角色头像等游戏素材需要统一尺寸规格，保持视觉一致性
// - **内容层次**：信息密度较高的区域需要合理使用间距和分组，避免视觉混乱
// - **交互反馈**：筛选、搜索等功能需要明确的状态反馈，提升用户体验

// **参考风格**
// Fandom游戏Wiki、灰机Wiki等游戏资料站点的布局和功能设计
// \`\`\`

// \`\`\`json title="星露谷物语Wiki组件选型.json"
// [
//   {
//     "namespace": "mybricks.normal-pc.antd5.custom-container"
//   },
//   {
//     "namespace": "mybricks.normal-pc.antd5.text"
//   }
// ]
// \`\`\``,
//     `基于特斯拉官网的需求，我来分析并搭建这个具有强烈科技感和未来感的页面：

// ## 总体分析
// 1. **功能定位**：展示特斯拉电动汽车产品、技术创新和品牌理念
// 2. **设计风格**：极简未来主义，黑白强对比，大量留白，突出科技感
// 3. **布局结构**：从上到下分为导航栏、英雄区域、车型展示、技术创新、充电网络、可持续发展、页脚等模块

// ## 组件选择与布局方案
// - 使用自定义容器作为主要布局组件，配置flex垂直布局
// - 导航栏采用水平布局，左中右三部分结构
// - 英雄区域使用全屏背景图片配合居中内容
// - 各功能区域采用合适的网格和卡片布局
// - 注重间距和视觉层次，体现特斯拉的极简美学

// ## 详细实现方案
// 按照从上到下的顺序，配置页面高度、布局方式，然后逐步添加各个功能模块，注重科技感的视觉效果和用户体验。

// \`\`\`json title="操作步骤.json"
// ["_root_",":root","setLayout",{"height":2400}]
// ["_root_",":root","doConfig",{"path":"root/标题","value":"分户成本确认表单"}]
// ["_root_",":root","doConfig",{"path":"root/布局","value":{"display":"flex","flexDirection":"column","alignItems":"center"}}]
// ["_root_",":root","doConfig",{"path":"root/样式","style":{"background":"#F5F5F5"}}]
// ["_root_","_rootSlot_","addChild",{"title":"表单容器","ns":"mybricks.normal-pc.antd5.form-container","comId":"u_mainForm","layout":{"width":"100%","height":"fit-content","marginLeft":16,"marginRight":16},"configs":[{"path":"表单容器/类型","value":"Form"},{"path":"表单容器/表单项布局/类型","value":"horizontal"},{"path":"表单容器/表单项布局/每行列数","value":3},{"path":"表单容器/表单项布局/列间距","value":16},{"path":"表单容器/表单项布局/尺寸","value":"middle"},{"path":"表单容器/标题/宽度类型","value":"px"},{"path":"表单容器/标题/标题宽度(px)","value":100},{"path":"表单容器/标题/显示冒号","value":true},{"path":"操作区/显示","value":false},{"path":"样式/表单/背景色","style":{"background":"#FFFFFF"}},{"path":"样式/默认/字体","style":{"fontSize":"14px","color":"#262626","lineHeight":"22px"}}]}]
// ["u_mainForm","content","addChild",{"title":"项目名称","ns":"mybricks.normal-pc.antd5.form-item-container","comId":"u_projectName","layout":{"width":"100%","height":"fit-content"},"configs":[{"path":":parent/表单项/标题","value":"项目名称："},{"path":":parent/表单项/样式/宽度模式","value":"span"},{"path":":parent/表单项/样式/宽度配置(共24格)","value":24}]}]
// ["u_projectName","formItem","addChild",{"title":"项目名称容器","ns":"mybricks.normal-pc.antd5.custom-container","comId":"u_projectNameCont","ignore":true,"layout":{"width":"100%","height":"fit-content"},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"row","alignItems":"center"}}]}]
// ["u_projectNameCont","content","addChild",{"title":"项目编号链接","ns":"mybricks.normal-pc.antd5.text","comId":"u_projectCode","layout":{"width":"fit-content","height":"fit-content","marginRight":8},"configs":[{"path":"常规/内容","value":"【项目编号】"},{"path":"样式/默认/默认","style":{"fontSize":"14px","color":"#1890FF","lineHeight":"22px","cursor":"pointer"}}]}]
// ["u_projectNameCont","content","addChild",{"title":"业务类型链接","ns":"mybricks.normal-pc.antd5.text","comId":"u_businessType","layout":{"width":"fit-content","height":"fit-content","marginRight":8},"configs":[{"path":"常规/内容","value":"项目简称-业务类型"},{"path":"样式/默认/默认","style":{"fontSize":"14px","color":"#1890FF","lineHeight":"22px","cursor":"pointer"}}]}]
// ["u_projectNameCont","content","addChild",{"title":"N级关联按钮","ns":"mybricks.normal-pc.antd5.custom-button","comId":"u_relationBtn","layout":{"width":"fit-content","height":32},"configs":[{"path":"按钮/文字标题","value":"N级关联"},{"path":"样式/风格","value":"default"},{"path":"样式/默认/按钮","style":{"fontSize":"14px","borderRadius":"4px","borderColor":"#D9D9D9"}}]}]
// ["u_mainForm","content","addChild",{"title":"项目团队","ns":"mybricks.normal-pc.antd5.form-text","comId":"u_projectTeam","layout":{"width":"100%","height":"fit-content"},"configs":[{"path":":parent/表单项/标题","value":"项目团队："},{"path":":parent/表单项/样式/宽度模式","value":"span"},{"path":":parent/表单项/样式/宽度配置(共24格)","value":24},{"path":"常规/提示内容","value":"部门-团队长团队负责（含n，n分团）"},{"path":"常规/禁用状态","value":false}]}]
// ["u_mainForm","content","addChild",{"title":"收购金额","ns":"mybricks.normal-pc.antd5.form-text","comId":"u_acquisitionAmount","layout":{"width":"100%","height":"fit-content"},"configs":[{"path":":parent/表单项/标题","value":"收购金额："},{"path":":parent/表单项/样式/宽度模式","value":"span"},{"path":":parent/表单项/样式/宽度配置(共24格)","value":8},{"path":"常规/提示内容","value":"请输入金额"},{"path":"常规/后置标签","value":"元"}]}]
// ["u_mainForm","content","addChild",{"title":"实际出资额","ns":"mybricks.normal-pc.antd5.form-text","comId":"u_actualInvestment","layout":{"width":"100%","height":"fit-content"},"configs":[{"path":":parent/表单项/标题","value":"*其中：实际出资额："},{"path":":parent/表单项/样式/宽度模式","value":"span"},{"path":":parent/表单项/样式/宽度配置(共24格)","value":8},{"path":"常规/提示内容","value":"请输入金额"},{"path":"常规/后置标签","value":"元"}]}]
// ["u_mainForm","content","addChild",{"title":"估值金额","ns":"mybricks.normal-pc.antd5.form-item-container","comId":"u_valuationAmount","layout":{"width":"100%","height":"fit-content"},"configs":[{"path":":parent/表单项/标题","value":"估值金额："},{"path":":parent/表单项/样式/宽度模式","value":"span"},{"path":":parent/表单项/样式/宽度配置(共24格)","value":8}]}]
// ["u_valuationAmount","formItem","addChild",{"title":"估值金额容器","ns":"mybricks.normal-pc.antd5.custom-container","comId":"u_valuationCont","ignore":true,"layout":{"width":"100%","height":"fit-content"},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"row","alignItems":"center"}}]}]
// ["u_valuationCont","content","addChild",{"title":"估值金额输入框","ns":"mybricks.normal-pc.antd5.form-text","comId":"u_valuationInput","layout":{"width":"100%","height":"fit-content"},"configs":[{"path":"常规/提示内容","value":"请输入金额"},{"path":"常规/后置标签","value":"元"}]}]
// ["u_valuationCont","content","addChild",{"title":"更新估值金额","ns":"mybricks.normal-pc.antd5.custom-container","comId":"u_updateValuation","enhance":true,"layout":{"width":"fit-content","height":"fit-content","marginLeft":8},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"row","alignItems":"center"}}]}]
// ["u_updateValuation","content","addChild",{"title":"更新图标","ns":"mybricks.normal-pc.antd5.icon","comId":"u_updateIcon","layout":{"width":16,"height":"fit-content","marginRight":4},"configs":[{"path":"常规/选择图标","value":"SyncOutlined"},{"path":"样式/默认/颜色","style":{"color":"#1890FF","fontSize":"16px"}}]}]
// ["u_updateValuation","content","addChild",{"title":"更新文本","ns":"mybricks.normal-pc.antd5.text","comId":"u_updateText","layout":{"width":"fit-content","height":"fit-content"},"configs":[{"path":"常规/内容","value":"更新估值金额"},{"path":"样式/默认/默认","style":{"fontSize":"14px","color":"#1890FF","lineHeight":"22px","cursor":"pointer"}}]}]
// \`\`\`
// `
//   ]
})


function systemAppendPrompts() {
  return `
<对于当前搭建有以下特殊上下文>
  <搭建画布信息>
    当前搭建画布的宽度为1024，所有元素的尺寸需要关注此信息，且尽可能自适应布局。1024只是在MyBricks搭建时的画布宽度，实际运行时可能会更宽。
    
    搭建内容必须参考PC端网站进行设计，内容必须考虑左右排列的丰富度，以及以下PC的特性
      比如:
        1. 布局需要自适应画布宽度，实际运行的电脑宽度不固定；
        2. 宽度和间距配置的时候要注意，画布只有1024，特别注意总宽度不可以超过1024；
        3. 页面可以配置backgroundColor；
  </搭建画布信息>

  <允许使用的图标>
  antd中的图标
  </允许使用的图标>

  <对于图片或原型>
    可能会存在明显异于UI的评论、标注信息，注意筛选后去除。
  </对于图片或原型>
</对于当前搭建有以下特殊上下文>`
}

function prdExamplesPrompts() {

  return `
<example>
  <user_query>我要搭建一个京东首页</user_query>
  <assistant_response>
  好的，我来参考京东首页的内容实现一下，以下是需求分析规格说明书和组件选型的内容：
${fileFormat({
  content: `*概述*
本文档旨在规划一个功能丰富、营销氛围浓厚的综合性电商平台PC端首页，用于吸引用户、展示商品并引导消费。

*设计规范*
- *设计风格*：整体采用现代营销风，在按钮、文字上以现代简约为主，但是在色彩和内容密度上提供大量营销氛围。
- *色彩体系*：根据京东品牌风格和营销的特质，主色调采用品牌红 \`#E83A30\`，用于强调按钮、活动标签等；辅助色为浅灰色 \`#F2F2F2\` 用于背景，文字主色为 \`#333333\`。
- *字号规范*：正文字号 \`12px\`，标题根据层级在 \`16px-20px\` 之间。
- *间距与圆角*：模块间距统一为 \`16px\`，大部分无圆角，视情况配置圆角。

*设计亮点*
- *丰富的促销内容区域*：在核心内容区设计了左中右三栏布局，提升信息密度，在中间还设计了大量的促销模块。
- *黑金尊贵感*：个人信息卡片采用黑金配色，突出PLUS会员的尊贵感。
- *丰富的商品卡片*：商品卡片信息丰富，包含图片、标题、价格、划线价、标签（百亿补贴）、优惠券等多维度信息，提升用户购买欲望。

*内容*
*顶部灰色快捷导航栏*
- *功能*：提供地理位置、用户信息及常用功能快捷入口。
- *视觉*：这是一个PC页面的顶部通栏，背景为 \`#f2f2f2\` 浅灰色，高度约 \`30px\`，可以尽量放在更多的内容。
- *内容*：
  - 左侧：一个定位图标 + “北京”文字链接，旁边是用户昵称文本，例如 “Hi, a-ming”。
  - 右侧：多个横向排列的文本链接，如“我的订单”、“我的京东”、“企业采购”，以及一个带图标的“购物车”入口。
*主头部区域（Logo与搜索）*
- *功能*：网站核心的品牌标识和全局搜索功能。
- *视觉*：位于快捷导航栏下方，以品牌红色为主，视觉突出，高度约 \`100px\`。
- *内容*：
  - 左侧：京东的红色Logo图片。
  - 中间：一个搜索栏。包含一个带有放大镜图标的“搜索”按钮，输入框内有灰色的提示文字，如“搜你想搜”。
  - 右侧：一个风格现代的“我的购物车”按钮，带有购物车图标和数量角标。
*核心内容区*
- *功能*：页面的核心，聚合了左侧分类导航、主要营销活动和右侧个人信息。
- *视觉*：经典的左中右三栏布局，占据页面主要空间。
- *内容*：
  - **左侧（商品分类）**：一个纵向的菜单，包含 “家用电器”、“手机通讯”、“电脑办公” 等多个文本分类。鼠标悬停时会弹出更详细的子分类面板。
  - **中间（轮播与频道）**：2*2的网格，按序号来看是一个活动轮播、京东秒杀、百亿补贴、9.9包邮。
    - 活动轮播：一个轮播图区域，展示当前的主要促销活动，图片自动轮播并可手动切换；
    - 京东秒杀：一个秒杀氛围卡片，包含倒计时和1*2的商品展示入口；
    - 百亿补贴：一个百亿补贴氛围卡片，包含1*2的商品展示入口，商品还有“已补贴XX元字样”；
    - 9.9包邮：一个9.9包邮氛围卡片，包含1*2的商品展示入口，有一些生鲜商品的绿色氛围。
  - **右侧（个人信息 + 服务入口）**：一个个人信息卡片，包含用户头像、昵称、“欢迎回来！”问候语。下方是多个2*3的图文结合的快捷服务入口（如“话费”、“机票”），整体具有**黑金质感**以体现PLUS会员的尊贵感。
*为你推荐（瀑布流商品列表）*
- *功能*：基于用户偏好推荐商品，无限滚动加载。
- *视觉*：一个两列或多列的、不等高的商品卡片陈列（瀑布流效果）。
- *内容*：
  - 顶部：一个标题“为你推荐”，右侧有一个“换一批”按钮。
  - 下方：无限滚动的商品信息网格。每个商品卡片包含：商品图片、多行截断的标题、优惠券标签（如“满199减100”）、红色价格、店铺名称链接和“看相似”按钮。

*数据需求*
- **需要接口数据的组件**：
  - 用户信息卡片：需要获取当前登录用户的头像、昵称、会员等级信息
  - 商品分类菜单：需要获取商品分类树形数据，支持二级分类展示
  - 轮播图组件：需要获取活动轮播图片、标题、跳转链接数据
  - 秒杀商品列表：需要获取秒杀商品数据，包含倒计时、价格、库存信息
  - 百亿补贴商品：需要获取补贴商品数据，包含补贴金额、原价、现价
  - 推荐商品列表：需要获取个性化推荐商品数据，支持分页加载
  - 购物车数量：需要获取当前用户购物车商品数量

*事件流程*
- **页面初始化事件**：
  - 页面加载时调用用户信息接口 → 更新用户信息显示和购物车数量
  - 调用商品分类接口 → 渲染左侧分类导航菜单
  - 调用轮播图接口 → 更新轮播图内容并开始自动播放
  - 调用秒杀商品接口 → 更新秒杀区域商品和倒计时
  - 调用推荐商品接口 → 渲染首屏推荐商品列表

- **用户交互事件**：
  - **搜索功能**：
    - 用户输入关键词 → 点击搜索按钮 → 跳转搜索结果页面并传递搜索参数
  - **分类导航**：
    - 鼠标悬停分类项 → 调用子分类接口 → 显示二级分类浮层
    - 点击分类项 → 跳转对应分类页面
  - **轮播图交互**：
    - 自动轮播：每3秒切换下一张图片
    - 手动切换：点击左右箭头或指示器切换图片
    - 点击轮播图 → 跳转活动页面
  - **商品卡片点击**：
    - 点击商品图片/标题 → 跳转商品详情页
    - 点击"看相似"按钮 → 跳转相似商品页面
    - 点击店铺名称 → 跳转店铺首页
  - **购物车操作**：
    - 点击购物车按钮 → 跳转购物车页面
    - 商品加入购物车后 → 更新购物车数量显示
  - **换一批功能**：
    - 点击"换一批"按钮 → 调用推荐接口获取新商品 → 刷新推荐列表
  - **无限滚动**：
    - 滚动到页面底部 → 自动调用下一页推荐商品接口 → 追加商品到列表

*参考风格*
京东(JD.com)、淘宝(Taobao.com) PC端首页。`,
  fileName: '京东首页需求文档.md',
  type: 'md'
})}

推荐采用以下组件进行搭建：
${fileFormat({
    content: `[
  {
    "namespace": "mybricks.somelib.icon"
  },
  {
    "namespace": "mybricks.somelib.text"
  },
  {
    "namespace": "mybricks.somelib.button"
  },
  {
    "namespace": "mybricks.somelib.list"
  },
  {
    "namespace": "mybricks.somelib.image"
  }
]`,
    fileName: 'XX页面所需要的组件信息.json'
  })}
  </assistant_response>
</example>
<example>
  <user_query>大学官网用下面这一份信息</user_query>
  <assistant_response>
  好的，我来为你生成一个现代风格的大学校园官网的需求文档和组件选型。
${fileFormat({
    content: `# 概述
设计一个现代风格的大学校园PC端的官网，展示学校的综合实力、学术氛围和校园文化。整体采用扁平化设计，注重信息层次和视觉冲击力。

# 设计规范
- *设计风格*：现代扁平化，注重科技感和活力，采用渐变色和毛玻璃效果卡片提升质感。
- *色彩体系*：根据风格和用户要求，主色调采用科技蓝作为主色，辅以白色和浅灰色作为背景，文字主色为深灰色。
- *字号规范*：正文字号 \`14px-16px\`，标题根据层级在 \`32px-48px\` 之间。
- *间距与圆角*：模块间距统一为 \`24px-40px\`，区块间距为 \`80px-120px\`，圆角视情况配置。

# 设计亮点
- *现代感*：根据设计风格，采用扁平化设计、渐变背景、毛玻璃效果、微动效，突出科技感和活力；
- *内容丰富*：
  - 合理利用空间，通过卡片、图标、数据可视化等方式提升信息展示效率；
  - 数据可视化展示学校实力，用图标和数字增强说服力
  - 不对称布局增加视觉趣味性和内容利用率
- **响应式考虑**：画布宽度\`1024px\`，所有元素不超出画布，采用灵活布局适配；

我们从上到下、从左到右来分析UI结构：

## 顶部导航栏
**功能**：提供全站导航和快速入口，展示学校标识
**视觉**：固定在页面顶部，背景为**半透明毛玻璃效果**，滚动后变为纯色，宽度\`1024px\`无左右间距
- **左侧**：学校logo（\`60*60px\`）+ 学校中英文名称（中文\`18px\`粗体、英文\`12px\`细体）
- **中间**：横向导航菜单，包含“首页”、“学校概况”、“院系专业”等文本入口（字号\`14px\`、间距\`32px\`）
- **右侧**：一个搜索图标按钮 + 一个语言切换文本（中/EN）+ 一个登录入口按钮
## 首屏英雄区（Hero Section）
**功能**：第一视觉冲击，展示学校形象和核心价值
**视觉**：占据首屏高度（\`600-800px\`），采用**渐变背景**（深蓝到紫色）+ 校园实景图叠加，内容居中
- **主标题**：**双色渐变**标题（\`48px\`超粗体），展示学校slogan
- **副标题**：一句话介绍（\`18px\`），如"一所具有百年历史的综合性研究型大学"
- **数据标签**：3-4个浮动的数据卡片，以特定位置悬浮在背景上，展示"QS排名Top 50"、"95%就业率"等亮点，采用**半透明背景 + 渐变边框**
- **行动按钮组**：两个按钮，主按钮（**渐变色填充**）+ 次按钮（**描边样式**），按钮均带图标
- **滚动提示**：底部居中有一个带**动画效果**的向下箭头，提示用户继续浏览
## 学校概况区
**功能**：简要介绍学校历史、定位和优势
**视觉**：左右不对称布局，左侧占40%，右侧占60%，区块上下间距\`100px\`
- **区块标题**：居中，包含主标题"学校概况"（\`36px\`粗体）+ 英文副标题"About Us"（\`14px\`细体灰色）
- **左侧内容**：
  - 一张历史图片（固定宽度\`360px\`、高度\`480px\`、圆角\`12px\`、带**阴影**）
  - 图片右下角悬浮一个小标签："始建于1920年"
- **右侧内容**：
  - 学校介绍文本（\`16px\`行高\`1.8\`，约200字），重点文字用主题色高亮
  - 介绍文本下方是一个包含四个横向均分卡片的区域，每个卡片包含：顶部的彩色图标（\`32*32px\`）、数据数字（\`48px\`粗体、主题色）、数据说明（\`14px\`灰色）。例如“办学历史120年”、“校园面积3000亩”等。
  - 最底部是一个"了解更多"的链接按钮（带右箭头图标）
## 院系专业区
**功能**：展示学科覆盖和王牌专业
**视觉**：标题 + 可切换的横向滑动卡片列表，区块间距\`100px\`
- **区块标题**：同上格式，"院系专业"+"Faculties & Programs"
- **分类标签**：一个可切换的标签栏，包含"全部"、"理工科"、"人文社科"等选项
- **学院卡片列表**：一个可横向滚动的列表，每行展示4个卡片。每个卡片包含：
  - 学院图片（宽\`240px\`高\`180px\`、圆角\`8px\`）
  - 学院名称（\`20px\`粗体）
  - 位于右上角的王牌标签（红色背景，文字“国家重点学科”）
  - 图标+数字信息：如教师数量、学生数量
  - 底部"了解详情"文本链接（带右箭头）
- **查看更多**：底部有一个居中的“查看更多”按钮
## 科学研究区
**功能**：展示科研实力和成果
**视觉**：左右不对称布局，左侧占60%，右侧占40%，区块间距\`100px\`
- **区块标题**：同上格式，"科学研究"+"Scientific Research"
- **左侧内容**：
  - 一个两行两列的网格，展示4个科研成果图片（每个宽\`280px\`高\`200px\`、圆角\`8px\`、带**鼠标悬停放大效果**）
  - 图片上叠加标题文字（半透明黑色背景、白色文字），如"国家重点实验室"
- **右侧内容**：
  - 科研介绍文本（\`16px\`、2-3段）
  - 三个竖向排列的数据卡片，采用**渐变边框**样式，包含图标和数字，如“国家重点实验室数量：8个”
  - 底部"科研成果展示"按钮
## 校园生活区
**功能**：展示丰富的校园文化和学生活动
**视觉**：采用三列、不等高的图片陈列（**瀑布流效果**），区块间距\`100px\`
- **区块标题**：同上格式，"校园生活"+"Campus Life"
- **介绍文本**：居中，简短介绍（\`16px\`、1-2行）
- **图片网格**：展示8-10张校园生活图片，图片上叠加分类标签（"体育活动"、"社团文化"等），**鼠标悬停时图片放大并显示详细说明文字**。
## 新闻动态区
**功能**：展示学校最新资讯和动态
**视觉**：左右布局，左侧为重点新闻，右侧为新闻列表，区块间距\`100px\`
- **区块标题**：同上格式，"新闻动态"+"News & Events"
- **左侧内容**（宽度约\`600px\`）：
  - 一个大图新闻卡片，包含新闻标题（\`24px\`粗体、2行截断）、摘要（\`14px\`灰色、3行截断）、发布时间、阅读量。
- **右侧内容**（宽度约\`380px\`）：
  - 一个包含4-5条新闻的列表，每条包含：小缩略图（\`80*80px\`）、标题（\`16px\`）、发布时间。
  - 底部"查看全部新闻"文本链接
## 页脚区
**功能**：提供详细信息、快速导航和联系方式
**视觉**：深色背景（深蓝或深灰\`#1a1a2e\`）、左右无间距、上下内边距\`60px\`
- **上半部分**：内容分为四列均分
  - **第1栏**：学校logo + 简介（白色80%透明度文字、\`14px\`）
  - **第2栏**：快速链接列表
  - **第3栏**：在线服务列表
  - **第4栏**：联系我们信息及社交媒体图标（微信、微博等）
- **分隔线**：一条浅灰色\`1px\`的线
- **下半部分**：居中，包含版权信息、备案号等（\`12px\`灰色）

# 数据需求
- **需要接口数据的组件**：
  - 顶部导航栏：需要获取学校基本信息（中英文名称、logo链接）
  - 首屏英雄区：需要获取学校slogan、副标题文案、数据标签列表（标题、数值、描述）
  - 学校概况区：需要获取学校详细介绍文本、历史图片链接、核心数据卡片列表（图标、数值、描述）
  - 院系专业区：需要获取学院分类标签数据、学院卡片列表数据（学院名称、图片、王牌标签、教师数量、学生数量、详情链接）
  - 科学研究区：需要获取科研成果图片列表（图片、标题）、科研介绍文本、科研数据卡片列表（图标、标题、数值）
  - 校园生活区：需要获取校园生活图片列表（图片、分类标签、详细说明文字）
  - 新闻动态区：需要获取重点新闻数据（标题、摘要、发布时间、阅读量、图片）和最新新闻列表（标题、发布时间、缩略图）
  - 页脚区：需要获取学校简介文本、快速链接数据、在线服务数据、联系信息、社交媒体账号信息

# 事件流程
- **页面初始化事件**：
  - 页面加载时调用学校基本信息接口 → 更新顶部导航栏的学校logo和名称
  - 调用首屏英雄区数据接口 → 更新slogan、副标题和数据标签
  - 调用学校概况接口 → 更新介绍文本、历史图片和核心数据卡片
  - 调用院系专业接口 → 渲染分类标签和默认分类下的学院卡片列表
  - 调用科学研究接口 → 更新科研成果图片、介绍文本和科研数据卡片
  - 调用校园生活接口 → 渲染校园生活图片瀑布流
  - 调用新闻动态接口 → 更新重点新闻和最新新闻列表
  - 调用页脚信息接口 → 更新页脚各栏内容

- **用户交互事件**：
  - **导航栏交互**：
    - 点击导航菜单项 → 滚动到对应区块或跳转对应页面
    - 点击搜索图标 → 显示搜索框并支持关键词搜索
    - 点击语言切换 → 切换页面语言（中/英文）
    - 点击登录按钮 → 跳转登录页面
  - **首屏英雄区交互**：
    - 点击数据标签卡片 → 可能跳转到对应数据详情页面
    - 点击行动按钮（如“申请入学”、“虚拟校园”） → 跳转对应功能页面
    - 点击滚动提示箭头 → 平滑滚动到学校概况区块
  - **院系专业区交互**：
    - 点击分类标签 → 调用对应分类的学院数据接口 → 刷新学院卡片列表
    - 点击学院卡片或“了解详情”链接 → 跳转该学院详细介绍页面
    - 点击“查看更多”按钮 → 跳转院系专业总览页面
  - **科学研究区交互**：
    - 鼠标悬停科研成果图片 → 触发放大效果
    - 点击科研成果图片 → 可能跳转该科研成果详情页面
    - 点击“科研成果展示”按钮 → 跳转科研专题页面
  - **校园生活区交互**：
    - 鼠标悬停校园生活图片 → 图片放大并显示详细说明文字
    - 点击校园生活图片 → 可能跳转该活动详情页面或相册
  - **新闻动态区交互**：
    - 点击重点新闻卡片 → 跳转该新闻详情页面
    - 点击新闻列表项 → 跳转对应新闻详情页面
    - 点击“查看全部新闻” → 跳转新闻列表页面
  - **页脚区交互**：
    - 点击快速链接 → 跳转对应页面
    - 点击在线服务 → 跳转对应服务平台
    - 点击社交媒体图标 → 打开对应社交媒体主页
  - **滚动交互**：
    - 滚动页面时 → 顶部导航栏背景从半透明变为纯色
    - 滚动到页面底部 → 可能需要加载更多内容（如新闻、图片等）

# 参考风格
现代的大学官网设计风格，如清华大学、麻省理工学院等。
`,
    fileName: '现代风格大学校园官网需求文档.md',
    type: 'md'
})}

推荐采用以下组件进行搭建：
${fileFormat({
    content: `[
  {
    "namespace": "mybricks.somelib.icon"
  },
  {
    "namespace": "mybricks.somelib.text"
  },
  {
    "namespace": "mybricks.somelib.button"
  },
  {
    "namespace": "mybricks.somelib.list"
  },
  {
    "namespace": "mybricks.somelib.image"
  }
]`,
    fileName: 'XX页面所需要的组件信息.json'
  })}
  </assistant_response>
</example>
<example>
  <user_query>我要做一个小说官网，风格紧凑</user_query>
  <assistant_response>
好的，我来为你生成一个典型的PC端小说官网的需求分析规格说明书和组件选型建议。整体风格强调信息密度高、排版紧凑、交互直接，符合主流网络文学平台（如起点中文网、纵横中文网）的设计习惯。

${fileFormat({
  content: `# 概述
本文档用于规划一个高信息密度、强功能导向的PC端小说阅读平台首页，目标是快速引导用户发现新书、追更热门作品，并提升点击转化率。

# 设计规范
- *设计风格*：**极致紧凑型布局**，以文字内容为核心，几乎不留白，模块边界清晰但无装饰性圆角。
- *色彩体系*：
  - 主背景色：纯白 \`#FFFFFF\`
  - 文字主色：深灰 \`#333333\`（正文）、黑色 \`#000000\`（标题）
  - 强调色：红色 \`#C00000\`（用于“新书”、“限免”、“更新”等标签）
  - 链接色：蓝色 \`#0066CC\`
- *字号规范*：
  - 页面标题：\`20px\` 粗体
  - 榜单/分类标题：\`14px\` 粗体
  - 小说标题：\`16px\` 常规
  - 作者/简介/状态：\`12px\`
- *间距与圆角*：
  - 模块内元素垂直间距仅 \`8px\`，模块间间距约 \`16px\`
  - **所有按钮、卡片、输入框均无圆角（**
  - 表格、列表采用细线分割（\`1px solid #eee\`）

# 设计亮点
- **高密度信息展示**：首页塞入排行榜、分类导航、新书推荐、限免专区、连载更新等多个模块，最大化利用屏幕空间。
- **强引导性标签系统**：大量使用“新”、“热”、“限免”、“完本”等红色/绿色标签，刺激点击。
- **表格化小说列表**：采用类Excel表格形式展示小说信息（含封面缩略图、书名、作者、状态、字数、更新时间），便于快速浏览。
- **弱装饰、纯功能导向**：去除一切非必要视觉元素（如阴影、渐变、动效），聚焦内容本身。

# 内容结构

## 顶部通栏
- 背景：白色
- 左侧：网站Logo（文字“小说网”+ slogan“百万好书，免费读”）
- 中间：主导航菜单（横向排列）：“首页”、“男生频道”、“女生频道”、“排行榜”、“完本专区”、“限免专区”
- 右侧：登录/注册入口 + “我的书架”链接

## 搜索与公告区
- 上半部：一个宽搜索框（无圆角），占满宽度，placeholder 为“书名 / 作者 / 关键词”
- 下半部：滚动公告条（红色背景 \`#FFF0F0\`，文字红色），如“《XXX》今日限免！”

## 主内容区（三栏布局：左窄 + 中宽 + 右窄）

### 左侧栏
- **分类导航**：垂直列表，无间距，每项带“>”箭头，如“玄幻”、“都市”、“言情”、“科幻”等
- **热门标签**：一行横向标签（“免费”、“完本”、“新书”、“VIP”），点击可筛选

### 中间主区
- **轮播横幅**（可选）：仅一张静态广告图，高度 90px，无自动播放
- **榜单模块**（多个并列）：
  - 每个榜单标题如“24小时热销榜”、“新书潜力榜”，右侧有“更多 >”链接
  - 榜单内容为编号 + 书名 + 作者（12px），共10行，无图片
- **新书推荐**：
  - 标题“最新上架”
  - 采用 2x3 网格，每个单元包含：
    - 左侧：60x80 小说封面（无圆角）
    - 右侧：书名（16px 蓝色链接）、作者（12px 灰色）、状态（“连载中”绿色标签）、简介（12px 截断两行）
- **连载更新**：
  - 表格形式，列包括：封面、书名、最新章节、更新时间
  - 每行高度仅 32px，紧凑排列

### 右侧栏
- **限免专区**：
  - 红底白字标题“今日限免”
  - 列出3本限免小说，每本含封面（60x80）、书名（红色）、“立即阅读”按钮（红色边框，无填充）
- **人气作家**：
  - 头像（40x40）+ 作家名 + 代表作（12px）
- **下载APP提示**：
  - 简单文字：“手机看书更方便，扫码下载APP”

## 页脚（简单信息）
- 背景：浅灰 \`#F5F5F5\`
- 内容：版权信息、ICP备案号、友情链接（“关于我们”、“联系我们”、“广告服务”）

# 数据需求
- **需要接口数据的组件**：
  - 搜索与公告区：需要获取实时搜索建议词、最新滚动公告内容及跳转链接。
  - 分类导航与热门标签：需要获取全站小说分类（含子分类）数据；需要获取当前热门/推荐的筛选标签数据。
  - 榜单模块（多个）：需要获取“24小时热销榜”、“新书潜力榜”等多个榜单的小说列表数据，包含排名、书名、作者、类型、热度/潜力值等。
  - 新书推荐模块：需要获取最新上架的小说列表数据，包含封面图、书名、作者、状态、简介、字数等。
  - 连载更新模块：需要获取最新更新的小说章节列表数据，包含小说封面、书名、最新章节名、更新时间等。
  - 限免专区模块：需要获取当日限免小说的列表数据，包含封面、书名、作者、简介、限免截止时间等。
  - 人气作家模块：需要获取人气作家推荐列表数据，包含作家头像、笔名、代表作、简介等。
  - 用户登录状态与“我的书架”：需要获取当前用户登录状态、昵称及书架最近更新信息。

# 事件流程
- **页面初始化事件**：
  - 页面加载时调用用户状态接口 → 更新顶部登录/注册区域及“我的书架”入口状态。
  - 调用分类与热门标签接口 → 渲染左侧栏导航及热门标签。
  - 调用各榜单接口 → 并行渲染中间区多个榜单模块。
  - 调用新书推荐接口 → 渲染“最新上架”2x3网格。
  - 调用连载更新接口 → 渲染“连载更新”紧凑表格。
  - 调用限免专区接口 → 渲染右侧栏“今日限免”模块。
  - 调用人气作家接口 → 渲染右侧栏“人气作家”模块。
  - 调用公告接口 → 初始化搜索框建议词、启动底部公告条滚动。

- **用户交互事件**：
  - **搜索功能**：
    - 用户在搜索框输入 → 实时调用搜索建议接口 → 在下拉框中展示建议结果。
    - 用户点击搜索按钮或回车 → 跳转至搜索结果列表页，并携带关键词参数。
  - **分类/标签导航**：
    - 点击左侧分类项（如“玄幻”） → 跳转至对应分类的小说列表页。
    - 点击“热门标签”（如“免费”） → 对当前页面主内容区（榜单、新书等模块）进行即时筛选，或跳转至带该标签筛选条件的列表页。
  - **榜单/模块交互**：
    - 点击榜单模块标题旁的“更多 >”链接 → 跳转至该榜单的完整详情页。
    - 点击榜单中的某本小说名 → 跳转至该小说的详情页/阅读页。
  - **书籍内容点击**：
    - 点击任意模块中的小说封面、书名或作者名 → 跳转至该小说的详情页。
    - 点击“限免专区”内的“立即阅读”按钮 → 直接跳转至该限免小说的阅读器页面。
    - 点击“连载更新”表格中的“最新章节”链接 → 跳转至该章节的阅读页。
  - **用户操作**：
    - 点击“登录/注册” → 弹出登录框或跳转至登录页。
    - 点击“我的书架” → 跳转至用户个人书架页面。
  - **公告交互**：
    - 点击滚动的公告文字 → 跳转至公告详情或关联的活动/书籍页面。

# 参考风格
起点中文网、纵横中文网、17K小说网等传统PC端小说平台。
`,
  fileName: 'PC端小说官网需求文档.md',
  type: 'md'
})}

推荐采用以下组件进行搭建：
${fileFormat({
  content: `[
  {
    "namespace": "mybricks.somelib.text"
  },
  {
    "namespace": "mybricks.somelib.image"
  },
  {
    "namespace": "mybricks.somelib.list"
  },
  {
    "namespace": "mybricks.somelib.input"
  },
  {
    "namespace": "mybricks.somelib.button"
  }
]`,
  fileName: '小说官网页面所需要的组件信息.json'
})}
  </assistant_response>
</example>
<example>
  <user_query>还原这个图片/设计稿</user_query>
  <assistant_response>
好的，我来为你还原这个图片中的内容。整体是一个中后台的查询表单+表格的审批页面。

${fileFormat({
  content: `# 概述
还原用户图片中的UI内容。

# 内容结构
- 顶部查询表单，包含发起人搜索框、部门下拉框、请假类型下拉框的查询，同时包含查询和取消两个按钮；
- 中间为表格，表格包含开始时间、结束时间、状态、请假天数、还有审批和取消的操作列。

# 数据需求
- **需要接口数据的组件**：
  - **查询条件数据**：
    - 部门下拉框：需要获取当前用户可见的组织部门列表数据，包含部门ID、部门名称。
    - 请假类型下拉框：需要获取系统定义的请假类型列表数据，包含类型ID、类型名称（如：年假、病假、事假）。
  - **审批列表数据**：
    - 审批表格：需要获取分页的审批记录列表数据。每条数据应包含：记录ID、发起人ID、发起人姓名、开始时间、结束时间、审批状态（待审批/已通过/已驳回）、请假天数、关联业务ID。
  - **当前用户数据**（可选）：
    - 用于权限校验或默认查询条件预填，如当前用户所属部门。

# 事件流程
- **页面初始化事件**：
  1. 调用部门列表接口 → 渲染部门下拉框选项。
  2. 调用请假类型接口 → 渲染请假类型下拉框选项。
  3. 调用审批列表接口（首次调用使用默认或空查询条件）→ 渲染初始审批表格数据，并初始化分页信息。

- **用户交互事件**：
  - **查询功能**：
    - 用户在表单中填写/选择查询条件（发起人、部门、请假类型）→ 点击“查询”按钮 → 将表单数据组装为查询参数 → 调用审批列表接口 → 清空原表格数据并渲染新数据。
    - 点击“取消”按钮 → 清空所有查询表单的已填内容 → 恢复为初始状态（可选项：同时触发一次使用空参数的查询，刷新为全部数据）。
  - **表格操作功能**：
    - 点击操作列的“审批”按钮 → 弹出审批操作浮层/跳转审批详情页 → 用户填写审批意见并提交 → 调用审批操作接口 → 接口成功后，刷新当前表格数据（或仅更新该行状态）。
    - 点击操作列的“取消”按钮（此处指“取消申请”或“撤销审批”）→ 弹出确认对话框 → 用户确认后，调用撤销或作废接口 → 接口成功后，刷新当前表格数据。
  - **分页与加载**：
    - 点击表格分页器的页码或下一页 → 携带当前查询条件和页码参数，调用审批列表接口 → 渲染对应页的表格数据。
    - （若设计为滚动加载）滚动到表格底部 → 自动调用下一页审批数据接口 → 将新数据追加到当前表格列表底部。
`,
  fileName: '还原审批页面文档.md',
  type: 'md'
})}

推荐采用以下组件进行搭建：
${fileFormat({
  content: `[
  {
    "namespace": "mybricks.somelib.text"
  },
  {
    "namespace": "mybricks.somelib.form"
  },
  {
    "namespace": "mybricks.somelib.table"
  },
  {
    "namespace": "mybricks.somelib.input"
  },
  {
    "namespace": "mybricks.somelib.button"
  }
]`,
  fileName: '还原审批页面所需要的组件信息.json'
})}
  </assistant_response>
</example>
`
}

function generatePageActionExamplesPrompts() {
  return `
<example>
  <user_query>搭建一个知乎个人中心页面</user_query>
  <assistant_response>
    首先，必须根据页面内容设置一个合适的页面的高度。
    其次，必须对页面布局设置一个合理的布局。
    然后
    基于用户当前的选择上下文，我们来实现一个知乎个人中心页面，思考过程如下：
    1. 搭建页面时一般用从上到下的搭建方式，我们推荐在页面最外层设置为flex的垂直布局，这样好调整位置；
    2. 将页面从上到下分成顶部信息、个人资料、核心内容三个部分；
    3. 顶部导航固定到页面顶部，不设置左右间距，内容为左侧菜单 + 右侧头像和消息私信入口；
    4. 个人资料区域是一个左右布局，左侧是头像、昵称、登记、性别等集合信息，右侧是编辑按钮；
    5. 核心内容也是左右布局，左侧是最近浏览记录，展示浏览/点赞的帖子（有特殊的已点赞标记），右侧分为上下两个部分，上面是个人成就，勋章等荣誉信息，下方是帮助中心、举报中心、关于知乎等页脚入口；

    ${fileFormat({
      content: `["_root_",":root","setLayout",{"height": 820}]
      ["_root_",":root","doConfig",{"path":"root/标题","value":"个人中心页面框架"}]
      ["_root_",":root","doConfig",{"path":"root/布局","value":{"display":"flex","flexDirection":"column","alignItems":"center"}}]
      ["_root_",":root","doConfig",{"path":"root/样式","style":{"background":"#F5F5F5"}}]
      ["_root_","_rootSlot_","addChild",{"title":"顶部导航","ns":"some.banner","comId":"u_top32","enhance":true,"layout":{"width":"100%","height":80,"marginTop":8,"marginLeft":12,"marginRight":12},"configs":[{"path":"常规/布局","value":{"display":"flex"}}]}]
      ["_root_","_rootSlot_","addChild",{"title":"个人资料","ns":"some.container","comId":"u_a2fer","enhance":true,"layout":{"width":"100%","height":"fit-content","marginLeft":8,"marginRight":8},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"row","justifyContent":"space-between","alignItems":"center"}}]}]
      ["u_a2fer", "content", "addChild",{"title":"头像","ns":"some.avatar","comId":"u_avat","layout":{"width":64,"height":64},"configs":[]}]
      ["u_a2fer", "content", "addChild",{"title":"用户信息","ns":"some.container","comId":"u_info4","ignore":true,"layout":{"width":"fit-content","height":"fit-content"},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"row","alignItems":"center"}}]}]
      ["_root_","_rootSlot_","addChild",{"title":"核心内容","ns":"some.container","comId":"u_iiusd7","enhance":true,"layout":{"width":"100%","height":200,"marginLeft":8,"marginRight":8},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"column"}}]}]
      ["u_iiusd7", "content", "addChild",{"title":"最近浏览","ns":"some.list","comId":"u_rev","layout":{"width":"100%","height":"fit-content"},"configs":[]}]
      ["u_iiusd7", "content", "addChild",{"title":"右侧容器","ns":"some.container","comId":"u_rcont","layout":{"width":"auto","height":"fit-content","marginTop":8},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"column"}}]}]
      ["u_rcont", "content", "addChild",{"title":"个人成就区域","ns":"some.container","comId":"u_achieveCont","enhance":true,"layout":{"width":"100%","height":"fit-content"},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"column"}}]}]
      ["u_rcont", "content", "addChild",{"title":"页脚区域","ns":"some.container","comId":"u_fcont","layout":{"width":"100%","height":"fit-content","marginTop":8},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"column"}}]}]`,
      fileName: '生成个人中心页面操作步骤.json'
    })}

    注意：
    - 可以的话，flex布局容器尽量用fit-content来计算内容高度。
    - 用户信息布局组件的父组件为布局组件，且仅承担布局功能，不承担样式、点击功能，我们添加ignore标记来优化。
    - 顶部导航、个人资料、个人成就区域、页脚区域的flex容器组件属于图文信息卡片，我们添加了enhance标记，优化flex布局的体验。
  </assistant_response>
</example>

<example>
  <user_query>搭建一个云服务器管理中后台页面</user_query>
  <assistant_response>
    基于用户当前的需求，我们来实现一个云服务器管理中后台页面，思考过程如下：

    分析下布局，这是一个经典的顶部导航+中间内容（左侧菜单 + 右侧内容）+ 底部页脚的中后台页面。
    必须先确认_root_的情况，必须先配置 页面高度 和 设置布局为垂直布局，然后开始搭建。
    
    首先，用一个左右布局的容器，完成顶部导航的左右部分内容；
    然后是中间内容，这是一个典型的左侧固定宽度菜单 + 右侧自适应内容的布局，我们用直接用容器来实现
      - 添加一个垂直的容器布局，注意图片上内容有左右和上下的间距，配置margin；
      - 添加左侧菜单部分，使用一个固定宽度的容器，配置合理的marginRight间距；
        - 添加左侧内容...
      - 添加右侧内容部分，使用一个自适应宽度（width=auto）的容器；
        - 添加右侧内容...
    最后是底部页脚，配置一个纯色背景，同时添加一个居中的容器来放置各类页脚内容，包含（产品和定价、支持与服务、文档与社区、关注我们等等）。

    \`\`\`json file="actions.json"
    ["_root_",":root","setLayout",{"height": 1600}]
    ["_root_",":root","doConfig",{"path":"root/布局","value":{"display":"flex", "flexDirection": "column"}}]
    ["_root_",":root","doConfig",{"path":"root/样式","style":{"background":"#f5f5f5"}}]
    ["_root_","_rootSlot_","addChild",{"title":"顶部导航","ns":"some.container","comId":"u_head","enhance": true,"layout":{"width":"100%","height":64},"configs":[{"path":"常规/布局","value":{"display":"flex", "justifyContent": "space-between", "alignItems": "center"}}]}]
    ["u_head","content","addChild",{"title":"左侧Logo容器","ns":"some.container","comId":"u_logoc","enhance": true,"layout":{"width":160,"height":"fit-content","marginLeft":12},"configs":[{"path":"常规/布局","value":{"display":"flex", "alignItems": "center"}}]}]
    ["u_logoc","content","addChild",{"title":"Logo","ns":"some.image","comId":"u_logo","layout":{"width":32,"height":32},"configs":[]}]
    ["u_head","content","addChild",{"title":"右侧内容","ns":"some.container","comId":"u_headr","layout":{"width":"fit-content","height":"fit-content","marginRight":12},"configs":[]}]
    ["_root_","_rootSlot_","addChild",{"title":"中间内容","ns":"some.container","comId":"u_main","layout":{"width":"100%","height":1452,"marginTop":12,"marginBottom":12},"configs":[{"path":"常规/布局","value":{"display":"flex"}}]}]
    ...TODO...
    ["u_main","content","addChild",{"title":"左侧菜单","ns":"some.container","comId":"u_bar","layout":{"width":200,"height":"fit-content","marginRight":12},"configs":[{"path":"常规/布局","value":{"display":"flex", "flexDirection": "column"}}]}]
    ["u_main","content","addChild",{"title":"右侧内容","ns":"some.container","comId":"u_rcont","layout":{"width":"auto","height":"fit-content"},"configs":[{"path":"常规/布局","value":{"display":"flex", "flexDirection": "column"}}]}]
    ["_root_","_rootSlot_","addChild",{"title":"底部页脚","ns":"some.container","comId":"u_foot","enhance": true,"layout":{"width":"100%","height":80,"backgroundColor":"#222222"},"configs":[{"path":"常规/布局","value":{"display":"flex", "justifyContent": "center", "alignItems": "center"}}]}]
    ...TODO...
    \`\`\`
  
    在上述内容中：
    我们遵循了以下关键事项：
    流程：从「根组件高度和布局配置」-> 从上往下分区开始搭建内容 -> 。
    特别注意点：
      - 在中间内容中的布局中，右侧内容使用了自适应内容宽度的布局，宽度设置为auto，左侧固定，完成了自适应页面宽度的要求；
      - 「左侧菜单」和「右侧内容」，使用了关键性的自适应宽度，我们不能打上ignore标记，「左侧Logo容器」很有可能也会有点击时间，不能打上ignore标记；
      - 「顶部导航」、「左侧Logo容器」、「底部页脚」为信息展示的flex布局，且没有ignore标记，可以打上enhance标记；
      
  </assistant_response>
</example>

<example>
  <user_query>添加一个一行三列的导航</user_query>
  <assistant_response>
    好的，一行三列的导航考察的是我们布局的关键知识，一行三列，就是均分布局，均分我们一般选择使用flex布局。
    所以提供一个flex容器，确定子组件的宽度，并将内容平铺上去。
    
    首先，必须根据页面内容设置一个合适的页面的高度。
    其次，必须对页面布局设置一个合理的布局。
    
    ${fileFormat({
      content: `["_root_",":root","setLayout",{"height": 360}]
      ["_root_",":root","doConfig",{"path":"root/标题","value":"一行三列的导航"}]
      ["_root_",":root","doConfig",{"path":"root/布局","value":{"display":"flex","flexDirection":"column","alignItems":"center"}}]
      ["_root_","_rootSlot_","addChild",{"title":"Flex容器","ns":"some.container","comId":"u_iiusd7","enhance": true,"layout":{"width":"100%","height":200,"marginLeft":8,"marginRight":8},"configs":[{"path":"常规/布局","value":{"display":"flex","flexDirection":"row","justifyContent":"space-between","alignItems":"center","flexWrap":"wrap"}}]}]
      ["u_iiusd7","content","addChild",{"title":"导航1","ns":"some.icon","comId":"u_icon1","layout":{"width":120,"height":120,"marginTop":8},"configs":[{"path":"样式/文本","style":{"background":"#0000FF"}}]}]`,
      fileName: '一行三列导航操作步骤.json'
    })}

  注意：
    - 这个Flex容器是根组件的直接子组件，并且很有可能后续提供点击事件，所以不允许添加ignore标记，同时他属于图文展示，可以添加enhance标记。
  </assistant_response>
</example>`
}